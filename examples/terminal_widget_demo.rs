            ...
            if event.what == EventType::Command {
                match event.command {
                    CM_ADD_LINE => {
                        ...
                        if let Some(terminal) = app.desktop.child_at_mut(0).as_any_mut().downcast_mut::<TerminalWidget>() {
                            let elapsed = start_time.elapsed();
                            let line = format!("[{:>6.2}s] Processing item {}...", elapsed.as_secs_f32(), line_counter);
                    ...
                    CM_ADD_BATCH => {
                        ...
                        if let Some(terminal) = app.desktop.child_at_mut(0).as_any_mut().downcast_mut::<TerminalWidget>() {
                            for i in 0..100 {
                                let elapsed = start_time.elapsed();
                                let line = format!("[{:>6.2}s] Batch processing item {}/100", elapsed.as_secs_f32(), i + 1);
                                terminal.append_line(line);
                            }
