...
// Dynamic Title Demo - demonstrates changing dialog title at runtime
            ...
            if event.what == EventType::Command {
                match event.command {
                    CM_UPDATE_TITLE => {
                        ...
                        if let Some(view) = app.desktop.window_at_mut(dialog_index) {
                            // Downcast from &mut dyn View to &mut Dialog using as_any_mut()
                            if let Some(dlg) = view.as_any_mut().downcast_mut::<Dialog>() {
                                dlg.set_title(titles[title_index]);
                            }
                        }
