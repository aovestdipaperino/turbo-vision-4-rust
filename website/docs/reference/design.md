...
In Borland, child views communicate with parents via raw owner pointers. In Rust, child-to-parent communication uses **event transformation** (the call stack). Palette chain traversal uses **QCell-based safe nodes** (see [PALETTE-SYSTEM-DESIGN.md](palette-design.md)). No `unsafe` code remains in the view system.
...
> [PALETTE-SYSTEM-DESIGN.md](palette-design.md) and
